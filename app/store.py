...
def activate_reward(reward_id):
    """Activate a reward (badge, theme, boost, analytics). Only one badge/theme/boost can be active at a time."""
...
def can_redeem(reward_id):
    reward = next((r for r in REWARDS if r["id"] == reward_id), None)
    if not reward:
        return False, "Reward not found."
    points = achievements.get_points()
    unlocked = [a["difficulty"] for a in achievements.get_unlocked_achievements()]
...
def redeem_reward(reward_id, update_cash_balance, get_cash_balance):
    reward = next((r for r in REWARDS if r["id"] == reward_id), None)
    if not reward:
        return False, "Reward not found."
    can, msg = can_redeem(reward_id)
    if not can:
        return False, msg
    if achievements.redeem_points(reward["cost"]):
        # Handle different reward types
        if reward["type"] == "cash":
            update_cash_balance(get_cash_balance() + reward["value"])
            _add_owned_reward(reward_id)
...
def _add_owned_reward(reward_id):
    owned = get_owned_rewards()
